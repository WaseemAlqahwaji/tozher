...
enum BaseStatus { initial, inProgress, success, failure }
...
class BaseState<T> {
  ...
  BaseState<T> setInProgressState() =>
      BaseState<T>(status: BaseStatus.inProgress);
  ...
  const BaseState({this.status = BaseStatus.initial, this.item, this.failure});

  BaseState<T> copyWith({BaseStatus? status, T? item, Failure? failure}) {
    return BaseState<T>(
